...
**Important:** JSON Schema `minimum`/`maximum` constraints are NOT enforced by constrained decoding — use WriteState `constraints` or StateStore `min`/`max` for number range enforcement.
