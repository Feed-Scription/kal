...
- 通过 stdio 或 SSE 传输暴露给 IDE Agent
- 详见本文档第十二章和 `ide-dev-guide.md` 附录 A
...
- **MCP Server 是推荐的集成方式**：
  ...
  - 通过 MCP 协议暴露给 IDE Agent（参考 Claude Code 33+ MCP 服务器）
...
packages/devkit-mcp-server/ (独立项目，参考 Claude Code MCP 架构)
├── package.json
├── tsconfig.json
├── src/
...
**MCP 集成架构**（参考 Claude Code MCP 架构）：
- **devkit 包**：纯逻辑层，可直接调用或通过 MCP 暴露
- **devkit-mcp-server 包**：独立项目，封装 devkit 为 MCP Server
  - 支持 stdio 传输（本地进程通信，参考 Claude Code 主要方式）
  - 支持 SSE 传输（远程 HTTP 通信，参考 Claude Code 远程模式）
  - 提供 30+ MCP Tools（参考 Claude Code 15+ 内置工具）
  - 实现事件通知机制（参考 Claude Code 8,884 个事件监听器）
- **IDE Agent**：通过 MCP Client 调用工具，驱动 Flow 开发
- 详见第十二章"MCP Server 开发指南"和 `ide-dev-guide.md` 附录 A
...
MCP Server 是 devkit 与 IDE 集成的推荐方式（参考 Claude Code MCP 架构）：

| 任务 | 内容 | 参考 |
|------|------|------|
| 5A | MCP Server 基础架构（2 天） | Claude Code MCP 实现 |
| 5B | FlowEditor Service + Tools（1 天） | Claude Code 工具系统 |
| 5C | Simulator/Replayer Service + Tools（1 天） | Claude Code Task 工具 |
| 5D | 事件通知 + 性能优化（1 天） | Claude Code 事件系统 |
...
## 十二、MCP Server 开发指南（参考 Claude Code MCP 实现）

### 12.1 为什么需要 MCP Server

**优势**（参考 Claude Code 设计理念）：

1. **解耦**：devkit 和 IDE 独立开发、独立部署
...
**完整的工具列表**（参考 Claude Code 15+ 内置工具）：
...
### 12.4 事件通知机制（参考 Claude Code 事件系统）
...
### 12.5 Service 设计模式（参考 Claude Code 服务架构）
...
### 12.6 错误处理（参考 Claude Code 2,512 个 try-catch）
...
```typescript
// 缓存 Flow JSON（参考 Claude Code 5 分钟缓存）
class FlowCache {
...
**IDE 配置文件**（参考 Claude Code settings.json）：
...
A: 不是必须的。devkit 可以直接在 Node.js 环境中使用。但如果要与 IDE Agent 集成，推荐使用 MCP Server（参考 Claude Code 架构）。

**Q: 如何处理 MCP Server 连接失败？**

A: 实现自动重连机制（参考 Claude Code 连接管理）：
...
A: MCP Server 支持两种传输方式（参考 Claude Code）：
...
A: 实现健康检查和自动重启（参考 Claude Code 错误恢复）：
