...
**左侧 — 节点库：**
- 按分类展示所有可用 Node（从 `GET /api/nodes` 获取）
- 分类：Signal、State、Prompt、Message、Text、Image、Data、Custom
- 拖拽节点到画布添加
- 搜索过滤

**中央 — 画布：**
- DAG 可视化编辑（基于 React Flow）
- 节点拖拽、缩放、平移
- Handle 连线（拖拽输出 Handle 到输入 Handle）
- 连线时实时类型检查（类型不匹配时高亮提示）
- 子 Flow 节点双击进入内部编辑
- 多选、复制、粘贴、撤销/重做
...
- 选中节点时显示：
  - 节点类型和标签
  - 输入 Handle 列表（可设置 defaultValue）
  - 输出 Handle 列表（只读）
  - Config 配置表单（基于 `configSchema` 动态生成）
- 选中连线时显示：
  - 源节点 / 源 Handle
  - 目标节点 / 目标 Handle
  - 类型信息
