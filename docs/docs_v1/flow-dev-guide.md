...
| 文件 | 职责 | 对外暴露 |
|------|------|----------|
| `interfaces.ts` | Flow、Node、Edge、Port、执行状态、事件、序列化、registry 等所有类型与接口 | 类型定义 |
| `flow-graph.ts` | DAG 构建、拓扑排序、环检测、写冲突检测、端口类型校验 | FlowGraph |
| `flow-executor.ts` | Flow 执行引擎 | FlowExecutor |
| `scheduler.ts` | 并发控制、优先级调度 | Scheduler（内部） |
| `node-runner.ts` | 单节点执行 | NodeRunner（内部） |
| `json-serializer.ts` | Flow JSON 序列化/反序列化、校验、handler 注册表、节点类型注册表 | FlowSerializer, HandlerRegistry, NodeTypeRegistry |
| `nodes/` | 内置节点类型（llm、prompt、state-read、state-write、json-parse、interact、timer、subflow） | 内置节点工厂 |
...
├── index.ts
├── interfaces.ts            # FlowDefinition, FlowJson, NodeDefinition, NodeContext,
│                            # Edge, PortConnection, FlowError, NodeError 等所有类型
├── flow-graph.ts            # DAG 构建、拓扑排序、环检测、写冲突检测
├── flow-executor.ts         # Flow 执行引擎
├── scheduler.ts             # 调度器（优先级 + 并发控制）
├── node-runner.ts           # 单节点执行
├── json-serializer.ts       # FlowJson 序列化/反序列化、校验
└── nodes/
    ├── index.ts
    ├── llm.ts               # 大模型调用（输入 messages，输出 response）
    ├── prompt.ts            # Prompt 组装（输入数据，输出 messages）
    ├── state-read.ts        # 读取 State 字段
    ├── state-write.ts       # 写入 State 字段
    ├── json-parse.ts        # JSON 解析与修复
    ├── interact.ts          # 等待用户输入
    ├── timer.ts             # 计时器触发
    └── subflow.ts           # SubFlow 嵌套
```
...
### 3.1 interfaces.ts — 节点与 Flow 类型

#### 节点类型（NodeDefinition, NodeContext, NodeKind...）
...
/**
 * 节点类型标识
 * - compute：自定义计算节点（开发者实现 run 函数）
 * - llm：大模型调用（内置，输入 messages，输出 response）
 * - prompt：Prompt 组装（内置，输入数据，输出 messages）
 * - state-read：读取 State 字段（内置，输出数据对象）
 * - state-write：写入 State 字段（内置，输入数据，无输出）
 * - json-parse：JSON 解析与修复（内置，输入 text，输出 json）
 * - interact：等待用户输入（内置）
 * - timer：计时器触发（内置）
 * - subflow：嵌套子 Flow（内置）
 */
...
export interface NodeCacheConfig {
  ...
  /** 覆盖 Flow 级或全局 poolPolicy；不填则向上继承 */
  readonly poolPolicy?: import('@kal-ai/core').PoolCachePolicy
  /** 节点级 cache key 策略；不填则使用全局默认 */
  readonly keyStrategy?: import('@kal-ai/core').CacheKeyStrategy
}
 ...
 *
 * 推荐数据获取方式：
 * - 上游节点数据：通过 ctx.inputs（端口-中心模式）或 ctx.deps（dependsOn 模式）
 * - State 读取：通过上游 state-read 节点的输出（ctx.inputs.data），或在 compute 节点中通过 ctx.core.state
 * - 直接访问 ctx.state 仍可用，但推荐通过 state-read 节点使数据流显式化
 */
export interface NodeContext {
  readonly nodeId: string
  /** 当前 State 快照（只读）；推荐通过 state-read 节点获取，使数据流在 DAG 中显式可见 */
...
#### Flow 定义（FlowDefinition, SubFlowRef...）

```typescript
import type { NodeDefinition } from './interfaces'
import type { PortConnection } from './interfaces'
...
export interface FlowDefinition {
  ...
  /** Flow 级缓存池策略，覆盖全局配置，被节点级覆盖；不填则继承全局 */
  readonly cachePolicy?: import('@kal-ai/core').PoolCachePolicy
}
...
#### 端口类型系统（PortDataType, PortDefinition, PortConnection...）

```typescript
// interfaces.ts（端口类型部分）
...
export type PortDataType =
  ...
  | 'video'        // 视频
  | 'mmc'          // MMC 多模态内容块
  | 'messages'     // ChatMessage[]（prompt 节点输出 / llm 节点输入）
  | 'response'     // ModelResponse（llm 节点输出）
...
#### 执行状态（NodeStatus, FlowStatus, NodeResult, FlowResult...）
...
```typescript
import type { NodeResult, FlowStatus } from './interfaces'
...
### 3.2 flow-graph.ts — DAG 构建与校验

```typescript
// flow-graph.ts（接口部分在 interfaces.ts）

import type { NodeDefinition, FlowDefinition } from './interfaces'
...
### 3.3 flow-executor.ts — Flow 执行引擎

```typescript
// flow-executor.ts（接口部分在 interfaces.ts）
...
```typescript
// nodes/index.ts（接口部分在 flow/interfaces.ts）

import type { NodeDefinition, SubFlowRef } from '../interfaces'
import type { PromptNode, ResolvedPrompt } from '@kal-ai/core'

// ======== llm 节点 — 大模型调用 ========
// 输入：messages（ChatMessage[]）
// 输出：response（ModelResponse）

export interface LlmNodeConfig {
  readonly id: string
  /** 使用的模型名（对应 KalCore 中注册的模型），不填则用默认模型 */
  ...
  /** 响应格式 */
  readonly responseFormat?: import('@kal-ai/core').ResponseFormat
  /** 是否流式输出 */
  readonly streaming?: boolean
  readonly dependsOn?: readonly string[]
  readonly priority?: import('../interfaces').NodePriority
  readonly cache?: import('../interfaces').NodeCacheConfig
}

/** 创建 llm 节点 */
export function createLlmNode(config: LlmNodeConfig): NodeDefinition<import('@kal-ai/core').ModelResponse>

// ======== prompt 节点 — Prompt 组装 ========
// 输入：data（任意数据对象，来自上游 state-read 或其他节点）
// 输出：messages（ChatMessage[]）

export interface PromptNodeConfig {
  readonly id: string
  /**
   * PromptNode 列表，由 compose() 组装。
   * 运行时调用 compose(nodes).resolve(ctx.inputs.data) 生成 messages。
   */
  readonly nodes: readonly PromptNode[]
  /** 格式化方式，默认 'xml' */
  readonly format?: 'xml' | 'markdown' | 'raw'
  readonly dependsOn?: readonly string[]
}

/** 创建 prompt 节点 */
export function createPromptNode(config: PromptNodeConfig): NodeDefinition<import('@kal-ai/core').ChatMessage[]>

// ======== state-read 节点 — 读取 State 字段 ========
// 输入：无
// 输出：data（Record<string, unknown>，包含所有读取的字段）

export interface StateReadNodeConfig {
  readonly id: string
  /**
   * 要读取的 State 路径列表。
   * 输出 data 对象的 key 为路径中最后一段（或 alias），value 为对应值。
   * 例：paths: ['player.hp', 'scene.narrative'] → { hp: ..., narrative: ... }
   */
  readonly paths: readonly (string | { path: string; alias: string })[]
  readonly dependsOn?: readonly string[]
}

/** 创建 state-read 节点 */
export function createStateReadNode(config: StateReadNodeConfig): NodeDefinition<Record<string, unknown>>

// ======== state-write 节点 — 写入 State 字段 ========
// 输入：data（来自上游节点，key 为 State 路径）
// 输出：无（void）

export interface StateWriteNodeConfig {
  readonly id: string
  /**
   * 写入映射：key 为输入端口名（对应上游输出的 key），value 为 State 路径。
   * 例：{ narrative: 'scene.narrative', hp: 'player.hp' }
   * 若不填，则将上游 data 对象的所有 key 直接作为 State 路径写入。
   */
  readonly mapping?: Readonly<Record<string, string>>
  /** 写入操作类型，默认 'set' */
  readonly op?: 'set' | 'merge'
  readonly dependsOn?: readonly string[]
  readonly writes?: readonly string[]  // 供写冲突检测使用
}

/** 创建 state-write 节点 */
export function createStateWriteNode(config: StateWriteNodeConfig): NodeDefinition<void>

// ======== json-parse 节点 — JSON 解析与修复 ========
// 输入：text（string，来自上游 llm 节点的 response.content）
// 输出：data（解析后的 JSON 对象）

export interface JsonParseNodeConfig {
  readonly id: string
  /** 是否启用 JSON 修复（处理截断、注释、尾逗号等），默认 true */
  readonly repair?: boolean
  /** 是否从 markdown 代码块中提取 JSON，默认 true */
  readonly extractFromCodeBlock?: boolean
  readonly dependsOn?: readonly string[]
}

/** 创建 json-parse 节点 */
export function createJsonParseNode(config: JsonParseNodeConfig): NodeDefinition<unknown>
...
/** 创建 subflow 节点 */
export function createSubFlowNode(config: SubFlowNodeConfig): NodeDefinition<import('../interfaces').FlowResult>
```

### 3.5 json-serializer.ts — Flow JSON 序列化、HandlerRegistry、NodeTypeRegistry

```typescript
// json-serializer.ts（接口部分在 interfaces.ts）

import type { FlowDefinition } from './interfaces'
import type { FlowGraphValidation } from './interfaces'
...
export interface FlowJson {
  ...
  /** Flow 级缓存池策略，覆盖全局配置，被节点级覆盖 */
  readonly cachePolicy?: PoolCachePolicy
}
...
export interface FlowNodeJson {
  readonly id: string
  readonly kind: 'compute' | 'llm' | 'prompt' | 'state-read' | 'state-write' | 'json-parse' | 'interact' | 'timer' | 'subflow'
  ...
  readonly cache?: {
    enabled: boolean
    ttlMs?: number
    /** 节点级缓存池策略（可序列化的纯数据部分）；keyStrategy 为函数，仅存在于运行时 NodeDefinition */
    poolPolicy?: PoolCachePolicy
  }
  ...
  /** prompt 节点配置（nodes 为函数引用，仅存在于运行时；JSON 中存 promptHandlerName） */
  readonly prompt?: {
    /** 引用 HandlerRegistry 中注册的 prompt 构建函数名 */
    readonly promptHandlerName: string
    readonly format?: 'xml' | 'markdown' | 'raw'
  }
  /** state-read 节点配置 */
  readonly stateRead?: {
...
export interface PortDefinitionJson {
  readonly name: string
  readonly type: import('./interfaces').PortDataType
  readonly description?: string
  readonly required?: boolean
  readonly defaultValue?: unknown
  readonly multiple?: boolean
...
export interface HandlerRegistry {
  register(name: string, handler: (ctx: import('./interfaces').NodeContext) => Promise<unknown>): void
  get(name: string): ((ctx: import('./interfaces').NodeContext) => Promise<unknown>) | undefined
  list(): readonly string[]
}
```

### 3.6 interfaces.ts — 依赖边（FlowEdge）
...
### 3.7 json-serializer.ts — 节点类型注册表（NodeTypeRegistry）

```typescript
// json-serializer.ts（NodeTypeRegistry 接口部分在 interfaces.ts）

import type { PortDefinition, PortDataType } from './interfaces'
import type { NodeKind } from './interfaces'
 ...
 * 内置节点类型（llm、prompt、state-read、state-write、json-parse、interact、timer、subflow）
 * 在框架初始化时自动注册。
 */
export interface NodeTypeRegistry {
  ...
  ): import('./interfaces').PortCompatibility
}
```
...
// 2. 注册内置节点类型（llm、prompt、state-read、state-write、json-parse、interact、timer、subflow）
...
registry.register({
  ...
  kind: 'compute',
  category: 'ai',
  inputs: [{ name: 'messages', type: 'messages', required: true }],
  outputs: [{ name: 'response', type: 'response' }],
...
// ---- nodes ----
export function createLlmNode(config: LlmNodeConfig): NodeDefinition<ModelResponse>
export function createPromptNode(config: PromptNodeConfig): NodeDefinition<ChatMessage[]>
export function createStateReadNode(config: StateReadNodeConfig): NodeDefinition<Record<string, unknown>>
export function createStateWriteNode(config: StateWriteNodeConfig): NodeDefinition<void>
export function createJsonParseNode(config: JsonParseNodeConfig): NodeDefinition<unknown>
export function createInteractNode(config: InteractNodeConfig): NodeDefinition<unknown>
...
/** 获取框架内置节点类型定义（llm、prompt、state-read、state-write、json-parse、interact、timer、subflow） */
...
> 说明：`Scheduler` 是 `FlowExecutor` 的内部实现细节，不对外暴露工厂函数。所有内置节点工厂函数创建的 `NodeDefinition` 会自动填充 `inputs`/`outputs` 端口定义，无需开发者手动声明。对应的端口元信息也包含在 `getBuiltinNodeTypes()` 返回的 `NodeTypeDefinition` 中。
...
│              @kal-ai/core/flow                     │
│                                                  │
│  interfaces.ts ◄── 所有文件都依赖                 │
│    ▲                                             │
│  flow-graph.ts ── 依赖 interfaces.ts             │
│    ▲                                             │
│  json-serializer.ts ── 依赖 interfaces + graph   │
│  scheduler.ts ── 依赖 interfaces.ts              │
│  node-runner.ts ── 依赖 interfaces + @kal-ai/core│
│  flow-executor.ts ── 依赖 interfaces + graph     │
│                     + scheduler + node-runner    │
│  nodes/ ── 依赖 interfaces + flow-executor       │
│                                                  │
│  对外：createFlowExecutor() 组装全部              │
│        createFlowSerializer() 供编辑器消费        │
└──────────────────────────────────────────────────┘
```

关键设计：内置节点（llm、prompt、state-read、state-write、json-parse）封装了最常见的 AI 调用管线，通过端口连接组合成完整的数据流。executor 通过 `KalCore` 获取 Model、StateManager 等能力，flow 模块本身不直接调用大模型，而是委托给 core。registry 模块为编辑器提供节点元信息，serialization 在反序列化时可通过 registry 校验端口连接合法性。
...
```typescript
// 单输出端口 — 使用内置 llm 节点（推荐）
createLlmNode({
  id: 'narrative',
  dependsOn: ['prompt'],
})
// llm 节点自动输出 { response: ModelResponse }，映射到 'response' 端口

// 单输出端口 — 自定义 compute 节点
{
  id: 'format',
  kind: 'compute',
  outputs: [{ name: 'text', type: 'text' }],
  run: async (ctx) => {
    const response = ctx.inputs.response as import('@kal-ai/core').ModelResponse
    return response.content  // 自动映射到 'text' 端口
  },
}
...
### 5.5.1 内置节点的 State writes 与写冲突检测

`state-write` 节点的 `writes` 字段用于静态写冲突检测，`compute` 节点的 `writes` 字段行为不变：

| 节点类型 | writes 作用 |
|---------|------------|
| `state-write` | 声明写入路径，供写冲突检测；实际写入由节点内部按 `mapping` 执行 |
| `compute` | 声明写入路径，供写冲突检测；框架按返回值自动写入（或 handler 内手动写入） |

**使用内置节点组合的典型 AI 调用管线：**

```typescript
import {
  createStateReadNode, createPromptNode, createLlmNode,
  createJsonParseNode, createStateWriteNode
...
const flow: FlowDefinition = {
  ...
  nodes: [
    ...
    createStateReadNode({
      id: 'read-state',
      paths: ['player', 'scene', { path: 'pending.action', alias: 'action' }],
    }),

    // 2. 组装 Prompt（数据来自上游 read-state 节点）
    createPromptNode({
      id: 'build-prompt',
      nodes: [
        base('system', '你是一个奇幻RPG的世界模拟器。根据世界状态和玩家动作，描述结果并以 JSON 返回状态变更。', { role: 'system' }),
        field('context', {
          source: data => [data],  // data 来自 read-state 输出
          describe: ([d]) => `世界状态：${JSON.stringify({ player: d.player, scene: d.scene })}\n动作：${d.action}`,
          role: 'user',
        }),
      ],
      format: 'raw',
    ...
    createLlmNode({
      id: 'llm',
      overrides: { responseFormat: { type: 'json_object' } },
      dependsOn: ['build-prompt'],
    }),
    ...
    // 5. 写回 State
    createStateWriteNode({
      id: 'write-state',
      mapping: {
        narrative: 'scene.narrative',
        patches: 'scene.simulationResult',
      },
      writes: ['scene.narrative', 'scene.simulationResult'],
      dependsOn: ['parse-response'],
    }),
  ],
  connections: [
    { fromNode: 'read-state',     fromPort: 'data',     toNode: 'build-prompt',   toPort: 'data' },
    { fromNode: 'build-prompt',   fromPort: 'messages', toNode: 'llm',            toPort: 'messages' },
    { fromNode: 'llm',            fromPort: 'response', toNode: 'parse-response', toPort: 'text' },
    { fromNode: 'parse-response', fromPort: 'data',     toNode: 'write-state',    toPort: 'data' },
  ],
}
```

**自定义 compute 节点仍可直接访问 KalCore：**

```typescript
// 需要更复杂逻辑时，compute 节点通过 ctx.core 访问全部能力
{
  id: 'custom-logic',
  kind: 'compute',
  run: async (ctx) => {
    const parsed = ctx.inputs.data as { patches: StatePatch[] }
...
import type { FlowDefinition } from './interfaces'
import type { FlowExecutor } from './interfaces'
...
const handlers = {
  ...
  // 注意：直接读 ctx.state 仍可用，但推荐通过上游 state-read 节点使数据流显式化
  validateCombatAction: async (ctx) => {
    const combat = ctx.inputs.data as { pendingAction: string }
    const player = ctx.inputs.data as { hp: number }
    if (player.hp <= 0) {
  ...
  // 2. 调用模型执行 AI 模拟（使用内置节点替代手动调用，见 5.5.1 示例）
  // 如需在 compute 节点中手动调用，仍可通过 ctx.core.model
  simulateCombatAction: async (ctx) => {
    const action = ctx.inputs.action as string
    const worldContext = ctx.inputs.worldContext as Record<string, unknown>
    const response = await ctx.core.model.invoke({
      messages: [
        { role: 'system', content: '你是一个奇幻RPG的世界模拟器。根据世界状态和玩家动作，描述结果并以 JSON 返回状态变更。' },
        { role: 'user', content: `世界状态：${JSON.stringify(worldContext)}\n动作：${action}` },
      ],
  ...
  renderCombatResult: async (ctx) => {
    const simResult = ctx.inputs.data as { narrative: string }
...
### Phase 0：类型定义（1 天）
- 完成 `interfaces.ts` 中所有类型与接口定义

### Phase 1：图构建（2 天，可与 core Phase 1 并行）
| 任务 | 模块 |
|------|------|
| 1A | flow-graph.ts — DAG 构建 + 拓扑排序 |
| 1B | flow-graph.ts — 环检测（含 SubFlow 引用） |
| 1C | flow-graph.ts — 写冲突检测 |

### Phase 2：执行引擎（3 天，依赖 core Phase 2 的 Model）
| 任务 | 前置依赖 |
|------|----------|
| 2A scheduler.ts — 优先级队列 + 并发控制 | Phase 1 |
| 2B node-runner.ts — 单节点执行 | core Model |
| 2C flow-executor.ts — 完整执行引擎 | 2A + 2B |

### Phase 3：内置节点 + 序列化（2 天，部分可并行）
| 任务 | 前置依赖 |
|------|----------|
| 3A nodes/llm.ts | Phase 2 + core Model |
| 3B nodes/prompt.ts | Phase 2 + core prompt |
| 3C nodes/state-read.ts + state-write.ts | Phase 2 + core State |
| 3D nodes/json-parse.ts | Phase 2 + core JsonRepair |
| 3E nodes/interact.ts | Phase 2 |
| 3F nodes/timer.ts | Phase 2 |
| 3G nodes/subflow.ts | Phase 2 + 1B(环检测) |
| 3H json-serializer.ts — JSON 序列化/反序列化 + NodeTypeRegistry | Phase 1 |
...
关键路径：`Phase 0 → 1A(拓扑排序) → 2C(执行引擎) → 3A(llm节点) → 4B(集成测试)`

## 八、最小可用链路（MVP）

跑通"state-read → prompt → llm → state-write 内置节点管线"：

```typescript
import { createKalCore } from '@kal-ai/core'
import {
  createFlowExecutor,
  createStateReadNode, createPromptNode, createLlmNode, createStateWriteNode
} from '@kal-ai/core/flow'
import { base, compose } from '@kal-ai/core'
...
executor.load({
  id: 'demo',
  nodes: [
    createStateReadNode({ id: 'read', paths: ['player'] }),
    createPromptNode({
      id: 'prompt',
      nodes: [
    ...
    createLlmNode({ id: 'llm', dependsOn: ['prompt'] }),
    createStateWriteNode({
      id: 'write',
      mapping: { content: 'scene.narrative' },
      writes: ['scene.narrative'],
      dependsOn: ['llm'],
    }),
  ],
  connections: [
    { fromNode: 'read',   fromPort: 'data',     toNode: 'prompt', toPort: 'data' },
    { fromNode: 'prompt', fromPort: 'messages',  toNode: 'llm',    toPort: 'messages' },
    { fromNode: 'llm',    fromPort: 'response',  toNode: 'write',  toPort: 'data' },
  ],
})
