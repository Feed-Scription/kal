...
| `flow` | AI 调用的 DAG 编排、并发控制、异步预生成 | FlowExecutor, FlowGraph, createFlowExecutor |
...
│       ├── types/
│       │   ├── index.ts
│       │   ├── flow.ts              # FlowDefinition, FlowJson
│       │   ├── node.ts              # NodeDefinition, NodeContext
│       │   ├── edge.ts              # Edge, PortConnection
│       │   └── errors.ts            # FlowError, NodeError
│       ├── graph/
│       │   ├── index.ts
│       │   ├── flow-graph.ts        # DAG 构建、拓扑排序
│       │   └── validator.ts         # 环检测、写冲突检测
│       ├── executor/
│       │   ├── index.ts
...
│       └── serializer/
│           ├── index.ts
│           └── json-serializer.ts   # FlowJson 序列化
└── __tests__/
    ├── model/
    ├── state/
    ├── prompt/
...
export function createJsonSerializer(handlers: HandlerRegistry): JsonSerializer
