...
| `prompt` | 函数式 prompt 片段组合与格式化，纯数据变换，不读 State | base, field, when, randomSlot, budget, compose |
...
| `flow` | AI 调用的 DAG 编排、并发控制、异步预生成；内置 LLM/Prompt/StateRead/StateWrite/JsonParse 节点 | FlowExecutor, FlowGraph, createFlowExecutor |
...
│   └── flow/                        # Flow 模块（DAG 编排）
│       ├── index.ts
│       ├── interfaces.ts            # FlowDefinition, FlowJson, NodeDefinition, NodeContext, Edge, FlowError 等
│       ├── flow-graph.ts            # DAG 构建、拓扑排序、环检测、写冲突检测
│       ├── flow-executor.ts         # Flow 执行引擎
│       ├── scheduler.ts             # 并发调度
│       ├── node-runner.ts           # 节点执行器
│       ├── json-serializer.ts       # FlowJson 序列化/反序列化
│       └── nodes/
│           ├── index.ts
│           ├── llm.ts               # LLM 调用节点
│           ├── prompt.ts            # Prompt 构建节点
│           ├── state-read.ts        # 读取游戏状态
│           ├── state-write.ts       # 写入游戏状态
│           ├── json-parse.ts        # JSON 后处理节点
│           ├── interact.ts          # 等待用户输入
│           ├── timer.ts             # 定时触发
│           └── subflow.ts           # 子 Flow 调用
└── __tests__/
    ├── model/
    ├── state/
    ├── prompt/
...
函数式 prompt 片段组合与格式化。核心概念是 `PromptNode`——可组合的 prompt 片段，通过 `compose` 组装后调用 `resolve(data)` 生成消息列表。

**设计原则：prompt 模块是纯数据变换，不直接读取 State。** State 的读取由 Flow 中的 `state-read` 节点完成，其输出作为数据传入 `compose().resolve(data)`。这使 prompt 模块可独立测试，也可在 Flow 之外使用。
...
export interface PromptNode {
  ...
  /** 估算 token 数（字符数 / 4 近似） */
  estimateTokens(data: Readonly<Record<string, unknown>>): number
  /** 解析为消息内容字符串（budget 不足时返回 null） */
  resolve(data: Readonly<Record<string, unknown>>): string | null
}
...
export interface FieldOptions<T = unknown> {
  /**
   * 从传入数据中提取原始数组。
   * data 是调用 resolve(data) 时传入的任意数据对象，
   * 通常来自上游 state-read 节点的输出。
   */
  readonly source: (data: Readonly<Record<string, unknown>>) => T[]
  /** 滑动窗口：取最近 N 条 */
...
export function when(
  id: string,
  condition: (data: Readonly<Record<string, unknown>>) => boolean,
...
export interface RandomSlotOptions {
  /**
   * 随机种子：
   * - 不填 / 'random'：每次 resolve 真随机（默认，适合生产环境多样性）
   * - number：固定种子，每次结果相同（适合测试和可复现场景）
   */
...
/**
 * 将节点列表组合为可解析的 prompt。
 * resolve(data) 接收任意数据对象，通常来自上游 state-read 节点的输出。
 */
export function compose(nodes: readonly PromptNode[]): {
  resolve(data: Readonly<Record<string, unknown>>): ResolvedPrompt
}
...
const prompt = compose([
  ...
  field('history', {
    source: data => data.events as Array<{ eventId: string; text: string; importance: number }>,
    window: 10,
    sample: 5,
  ...
  when('combat', data => Boolean(data.inCombat), [
  ...
  budget([
    field('npc_memory', {
      source: data => data.npcs as Array<{ npcId: string; memory: string }>,
      window: 20,
      sample: 5,
...
// data 由调用方提供，通常来自 StateManager.getSnapshot() 或 state-read 节点输出
const data = { events: [...], inCombat: true, npcs: [...] }
const resolved = prompt.resolve(data)
...
export interface CacheEntry<T = unknown> {
  readonly key: string
  readonly values: readonly T[]   // 多值池，支持多样性策略；单值场景取 values[0]
...
export interface CacheLayer<T = unknown> {
  ...
  /** 向池中追加一个新值（不覆盖已有值） */
  append(key: string, value: T, ttlMs?: number): Promise<void>
  /** 从池中随机返回一个值 */
  getRandom(key: string): Promise<CacheEntry<T> | null>
  /** 返回当前池中的值数量 */
  getPoolSize(key: string): Promise<number>
  delete(key: string): Promise<void>
...
// ---- 缓存池策略 ----
// 控制每次请求是从缓存池中随机取值，还是重新生成并写入池。
// 优先级：节点级 > Flow 级 > 全局级，不配置则退化为传统单值缓存行为。

/** 池满 minSize 个结果后才开始复用，否则始终重新生成 */
export interface MinPoolSizePolicy {
  readonly type: 'min-pool-size'
  readonly minSize: number        // 池中至少有 N 个结果才命中缓存
  readonly maxSize?: number       // 池满后不再写入新结果（可选）
}

/** 以固定概率跳过缓存直接重新生成，生成结果仍写入池 */
export interface ProbabilisticPolicy {
  readonly type: 'probabilistic'
  readonly bypassProbability: number  // 0~1，重新生成的概率
  readonly maxSize?: number
}

/** 组合策略：池不足 minSize 时必须生成；池够了仍以 bypassProbability 概率刷新 */
export interface CompositePolicy {
  readonly type: 'composite'
  readonly minSize: number
  readonly bypassProbability: number
  readonly maxSize?: number
}

export type PoolCachePolicy = MinPoolSizePolicy | ProbabilisticPolicy | CompositePolicy

// 中间件执行逻辑（伪代码）：
// 1. 生成 cache key
// 2. poolSize = cache.getPoolSize(key)
// 3. policy.decide(poolSize) → 'use-pool' | 'generate-new'
//    - 'use-pool'     → cache.getRandom(key) 直接返回
//    - 'generate-new' → 走 HTTP 调用，结果 append 到池（若未超 maxSize）
...
export interface CacheManagerOptions {
  ...
  /** 全局缓存池策略，可被 Flow 级或节点级覆盖；不配置则退化为传统单值缓存 */
  readonly poolPolicy?: PoolCachePolicy
}
...
export interface KalCore {
  ...
  /** 直接调用模型（接受 messages，返回 response） */
...
```
请求 → Observer(before) → Cache(lookup/pool-decide) → Retry(wrap) → Safety(input)
     → 用户自定义中间件 → HTTP调用 → Safety(output) → PostProcess(JSON repair)
     → Cache(append-to-pool) → Observer(after) → 返回响应
```

Cache 中间件在 lookup 阶段调用 `poolPolicy.decide(poolSize)`：
- `'use-pool'`     → `getRandom(key)` 直接返回，跳过后续所有中间件
- `'generate-new'` → 继续向后执行，响应返回后 `append` 到池（未超 maxSize 时）
...
│  prompt ── 依赖 types（纯数据变换，不依赖 state）  │
│  tools ── 依赖 types + model                     │
│  safety ── 依赖 types（通过 ModelMiddleware 注入） │
│  observe ── 依赖 types（通过 ModelMiddleware 注入）│
│  flow ── 依赖 types + model + state + tools      │
│    ▲                                             │
│  KalCore ── 组装全部模块（包括 flow）              │
└──────────────────────────────────────────────────┘
```

关键设计：model 通过 `ModelMiddleware` 定义中间件链，infra/safety/observe 通过 `asMiddleware()` 注入，运行时可选，编译时不硬依赖。prompt 模块是纯数据变换，不依赖 state，State 读取由 flow 中的 `state-read` 节点负责。
...
|-----------------|-------------------|
| 调用大模型 | 内置 `llm` 节点（封装 `KalCore.model`） |
| 读取游戏状态 | 内置 `state-read` 节点（封装 `KalCore.state.get`） |
| 写入游戏状态 | 内置 `state-write` 节点（封装 `KalCore.state.set/applyPatches`） |
| 构建 prompt | 内置 `prompt` 节点（封装 `compose()`/`formatXML()` 等，数据来自上游端口） |
| JSON 后处理 | 内置 `json-parse` 节点（封装 `KalCore.jsonRepair`） |
| 注册/调用 Tools | `KalCore.tools`（在 `llm` 节点或自定义 handler 中使用） |
| 缓存模型响应 | `KalCore.cache`（通过 middleware 自动生效） |

内置节点覆盖了最常见的 AI 调用管线，自定义 `compute` 节点通过 `NodeContext.core` 访问全部 KalCore 能力。
...
| 3E | flow — FlowGraph + FlowExecutor | 2A + 1F + 2D |
| 3F | flow/nodes — llm + prompt + state-read + state-write + json-parse 内置节点 | 3E + 1G |

### Phase 4：集成（2 天）
| 任务 | 模块 |
|------|------|
| 4A | KalCore 入口类 — 组装所有模块（包括 flow） |
| 4B | 集成测试 — 最小可用链路 |
| 4C | 导出整理 — index.ts barrel exports |
| 4D | 构建配置 — dual ESM/CJS 输出（tsup） |

关键路径：`Phase 0 → 1B(retry) → 2A(model) → 2E(function calling) → 3E(flow) → 4A(KalCore)`

### 人力分配建议（3 人）
| 开发者 | Phase 1 | Phase 2 | Phase 3 | Phase 4 |
|--------|---------|---------|---------|---------|
| A（核心） | infra/retry + cache | model 全部 | cache L3 + flow | KalCore 入口 |
| B | state + prompt | tools | MCP 客户端 | 集成测试 |
| C | json-repair + post-processor | cache L2 | safety + observe | 构建配置 |

注：flow 模块由 A 负责，因为需要深度理解 model、state、tools 的集成。
...
// prompt 模块是纯数据变换，data 由调用方提供
import { base, field, compose } from '@kal-ai/core'

const prompt = compose([
  base('system', '你是一个奇幻RPG的叙事AI。', { role: 'system' }),
  field('player', {
    source: data => [data.player],
    describe: ([player]) => `玩家状态: ${JSON.stringify(player)}，描述新手村。`,
    role: 'user',
  }),
])

const data = kal.state.getSnapshot()
const { messages } = prompt.resolve(data)
const response = await kal.model.invoke({ messages })
kal.state.set('scene.description', response.content)
```

在 Flow 中使用内置节点完成同样的事：

```typescript
import { createFlowExecutor, createStateReadNode, createPromptNode, createLlmNode, createStateWriteNode } from '@kal-ai/core/flow'
import { base, field, compose } from '@kal-ai/core'

const executor = createFlowExecutor({ core: kal })
executor.load({
  id: 'describe-scene',
  nodes: [
    ...
    createPromptNode({
      ...
      nodes: [
        ...
        field('player', {
          source: data => [data.player],
          describe: ([p]) => `玩家状态: ${JSON.stringify(p)}，描述新手村。`,
          role: 'user',
        }),
      ],
      dependsOn: ['read'],
    ...
    createStateWriteNode({ id: 'write', mapping: { content: 'scene.description' }, writes: ['scene.description'], dependsOn: ['llm'] }),
  ],
  connections: [
...
```json
{
  ...
  "exports": {
    ...
    "./flow": {
      "import": "./dist/esm/flow/index.js",
      "require": "./dist/cjs/flow/index.cjs",
...
// ---- cache pool policy types（供 flow 节点级配置使用） ----
export type { PoolCachePolicy, MinPoolSizePolicy, ProbabilisticPolicy, CompositePolicy }
