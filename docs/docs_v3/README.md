...
- **[core.md](./core.md)** - Core 模块详细设计（Node、Flow、State、基础设施）
- **[evolution.md](./evolution.md)** - 设计演进历程（V1 → V2 → V3）和适用场景

## 核心概念速览

### Node（节点）

功能实现的最小单位，通过 Handler（输入/输出端口）连接形成数据流。
...
- **可观测性**：自动记录每次 LLM 调用的详细信息（token 用量、耗时、缓存命中）
...
```json
{
  ...
  "edges": [
    { "source": "input", "sourceHandler": "text", "target": "message", "targetHandler": "user" },
    { "source": "message", "sourceHandler": "messages", "target": "llm", "targetHandler": "messages" },
    { "source": "llm", "sourceHandler": "text", "target": "output", "targetHandler": "text" }
  ]
}
```
...
- [V1 设计文档](../docs/core-dev-guide.md)（全功能 SDK）
- [V2 设计文档](../docs_v2/core.md)（节点式工作流引擎）
- [演进历程](./evolution.md)（V1 → V2 → V3 的变化）
