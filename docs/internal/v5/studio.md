...
因此，本文的结论比前一版更明确：KAL Studio 应采用 `Engine Platform Services + Studio Kernel + Built-in Core Views + First-party Extensions + Third-party Extensions` 的分层结构。`Flow` 与 `Session` 在 Phase 1 仍应作为 built-in core views，直接压测 Kernel 的 resource/document/transaction、diagnostics、inspector 和 run/debug 接口。`problems`、`prompt-preview`、`debugger`、`state-editor`、`config-editor`、`terminal`、`h5-preview`、`vercel-deploy`、`template-browser`、`package-manager` 等官方能力则应尽早改写为 first-party extensions，以真实压力检验 view registry、command bus、event stream、layout restoration 和 capability 模型是否足够稳定。与此同时，Engine 作为游戏运行时必须保持 Studio-optional：Studio 可以消费 Engine，但 Engine 在 `kal serve`、`kal play`、`kal debug`、`kal smoke` 等场景下不应以任何形式依赖 Studio 才能启动。
...
2. Studio 的工作台表面应如何开放。是把所有能力都降格为插件贡献，还是明确保留 built-in core views 承接当前最重的领域编辑器。
...
更重要的是，Theia 区分了产品级扩展与受限插件。其 extensions 更像编译时集成、可访问更深层容器的产品模块；plugins 则更接近受限 API 的运行时插件。这种区分对 KAL 很关键，因为我们当前确实既需要一层可演进的 Kernel / built-in core views，也需要可安装的 studio extensions。若一开始就把两者混成同一抽象，平台边界既会过宽，又会过脆。
...
3. 重型核心视图在平台早期应保留为 built-in core views，而不是被迫“伪装成插件”。
...
Built-in Core Views 只承接当前最重、最依赖语义模型的 Flow / Session。

First-party Extensions 负责用真实产品能力反向压测平台 API。
...
Browser Shell
  - Workbench layout
  - Built-in core views rendering
  - Lightweight first-party panels
...
### 6.3 为什么 Flow / Session 在 Phase 1 不应先插件化
...
因此，`kal.flow-editor` 与 `kal.session-editor` 在 Phase 1 应定义为 built-in core views。它们的目标不是永久留在内核，而是在最脆弱的阶段帮助 Kernel 找准边界。

只有当以下条件同时满足时，才应评估是否把它们迁移为 first-party extensions。
...
2. `Command API`：应用 transaction、运行 flow、推进 session、接受 proposal、创建 review、安装包、发布模板、执行 deploy。
3. `Event Stream`：传播 `project.reloaded`、`resource.changed`、`diagnostics.updated`、`run.updated`、`trace.appended`、`process.output`、`package.install.progress`、`review.changed` 等持续事件。
...
- 左侧：project explorer、nodes、templates、packages、comments、outline
...
### 7.2 First-party Extensions：官方能力是平台 API 的压力测试

在 Flow / Session 之外，Studio 的绝大多数官方能力都更适合以 first-party extensions 的形式交付。第一批最适合 dogfood 的官方扩展包括：
...
这批能力覆盖了 Studio 最需要验证的扩展表面：panel/view 注册、inspector 接入、command bus、run/debug service、event stream、long-running job progress、layout restoration、capability gate。只要这些官方扩展还不得不频繁绕过平台边界访问私有状态，就说明 Kernel API 还不成熟。
...
2. P2：加入 comments、presence、who-is-looking-at-what、review assignment。
3. P3：如果真的有必要，再评估局部 CRDT 或 shared editing 层，但 canonical persistence 仍通过 Engine transaction 落盘。
...
- 将 Flow / Session 以 built-in core views 的方式收进 Kernel
...
### 10.2 Phase 2：First-party Dogfooding

目标：用官方扩展压测平台边界，而不是直接开放第三方生态。

重点：

- 将 State / Config / Problems / Prompt Preview / Debugger 改造成 first-party extensions
...
- 让 `terminal`、`h5-preview`、`vercel-deploy` 继续以官方扩展形式接入
...
- 引入 comments、presence、review assignment
...
4. proposal bundle 是否需要独立资源类型，还是 transaction batch 的富化视图。
5. 是否要为 task presets 提供项目级共享能力，以便团队共享 `debug` / `review` 工作区布局。
...
KAL Studio 的正确方向，不是把现有 Editor 直接推向“大而全 IDE”，也不是把所有能力立刻做成插件。更合理的路径是：先以 Kernel 稳住语义资源、事务、诊断、布局、恢复和信任模型；再把 Flow / Session 作为 built-in core views 承接现有重型视图；再让官方能力以 first-party extensions 的方式 dogfood 平台 API；最后才逐步开放第三方生态。
