...
因此，本文的结论比前一版更明确：KAL Studio 应采用 `Engine Platform Services + Studio Kernel + Built-in Core Views + First-party Extensions + Third-party Extensions` 的分层结构。`Flow` 与 `Session` 在 Phase 1 仍应作为 built-in core views，直接压测 Kernel 的 resource/document/transaction、diagnostics、inspector 和 run/debug 接口。`problems`、`prompt-preview`、`debugger`、`state-editor`、`config-editor`、`version-control`、`terminal`、`h5-preview`、`vercel-deploy`、`template-browser`、`package-manager` 等官方能力则应尽早改写为 first-party extensions，以真实压力检验 view registry、command bus、event stream、layout restoration 和 capability 模型是否足够稳定。与此同时，Engine 作为游戏运行时必须保持 Studio-optional：Studio 可以消费 Engine，但 Engine 在 `kal serve`、`kal play`、`kal debug`、`kal smoke` 等场景下不应以任何形式依赖 Studio 才能启动。
...
在 Flow / Session 之外，Studio 的绝大多数官方能力都更适合以 first-party extensions 的形式交付。但这些能力不必在同一阶段迁移；更一致的做法是按依赖强度和阶段目标分组 dogfood。
...
- 接入 `comments`、`review`、`h5-preview`、`terminal`、`vercel-deploy` 等以验证工作流为中心的一方扩展
