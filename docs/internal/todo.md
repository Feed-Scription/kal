...
- [x] 文档 / Skills 规范性写作：格式已统一（H1 标题 → 一句话描述 → 正文 → See Also），重复文档已合并（3 对 → 保留 guides/ 路径），docs/README.md 已补全链接，代码块语言标记已修正
  剩余：extension-api.md / extension-guide.md 仍为中文，其余为英文。语言统一属内容决策，暂不处理。
