...
6. **Design for ApplyState** — State keys that LLM output should update must exist in initial state. `ApplyState` only modifies existing keys.
...
| State | `ReadState` / `ModifyState` / `ApplyState` / `AddState` / `RemoveState` | Read/write game state |
...
JSONParse (extract structured data from LLM response)
    ↓
ApplyState (write stateChanges back to StateStore)
    ↓
...
Let LLM return structured JSON with a `stateChanges` object, then use `ApplyState` to safely write back:
...
JSONParse extracts the object
    ↓
ApplyState config: { "path": "stateChanges", "allowedKeys": ["health", "gold", "currentLocation"] }
    ↓
...
2. **AI generation + rule validation** — LLM generates creative content, game rules (via ApplyState allowedKeys, custom nodes) validate and constrain it. Never let LLM directly set arbitrary state.
...
- Validation logic that goes beyond ApplyState's allowedKeys
...
- [ ] ApplyState only updates allowed keys (no unexpected state mutations)
...
| State not updating | ApplyState allowedKeys missing the key | Add the key to allowedKeys whitelist |
