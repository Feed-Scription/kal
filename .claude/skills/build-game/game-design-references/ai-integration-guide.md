...
JSONParse（解析 LLM 的 JSON 输出，启用 fixCommonErrors）
    ↓
ApplyState（回写 stateChanges，allowedKeys 白名单约束）
```

**关键约束**：
- `ApplyState` 的 `allowedKeys` 限制 LLM 只能修改指定的 state key
- `PromptBuild` 的 `base` fragment 中明确要求数值范围（如"每项变化不超过 ±20"）
- 重大事件（死亡、胜利等）由 Session 的 `Branch` 步骤二次判定，不完全依赖 LLM 输出

**反模式**：绝不要让 LLM 直接决定任意游戏状态。所有 LLM 输出必须经过 JSONParse + ApplyState 的结构化管道。
...
以下场景在 KAL 中均通过 `PromptBuild → Message → GenerateText → JSONParse → ApplyState` 节点链实现，区别在于 prompt 设计和输出 schema。

### 场景 1：AI 回合模拟

让 AI 生成每回合的结算叙事，而非只显示"+5 学术, -3 心态"。

**KAL 实现**：在主回合 Flow 中，用 `PromptBuild` 的 `field` fragment 注入当前状态，`base` fragment 描述输出 JSON schema，`GenerateText` 调用 LLM，`JSONParse` 解析结果，`ApplyState` 回写 stateChanges。

**验证要点**：
- `ApplyState` 的 `allowedKeys` 白名单限制可修改的 state key
...
用 `GenerateText` + `JSONParse` 生成结构化角色档案，通过 `ApplyState` 写入角色相关 state key。
...
动态生成游戏内商品。在 prompt 中描述商品 schema 和约束（价格范围、效果范围、智商税概率），`JSONParse` 解析后通过 `ApplyState` 写入 inventory 等 state。
...
- [ ] ApplyState 配置了 `allowedKeys` 白名单
...
- [ ] 验证 ApplyState 不会写入 allowedKeys 之外的 state key
