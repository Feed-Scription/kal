...
开箱即用：包含运行时、Engine 宿主、Studio 工作台和示例游戏。
...
- **Node manifest** — Engine 导出节点清单，Studio 工作台直接消费
- **热重载** — 开发时实时预览和调试
...
```bash
# 方式 A：Studio 模式（一条命令启动 engine + studio）
...
# 端点
GET    /api/project              # 提供给 Studio / 工具的项目快照
GET    /api/flows               # 列出 Flows
GET    /api/session             # 读取 Session 定义
...
GET    /api/runs/:id/state      # 查看 managed run 状态
GET    /api/runs/:id/stream     # 订阅 managed run 事件（SSE）
```
...
| [TODO](./docs/internal/todo.md) | 项目待办清单 |
...
- 17 个内置节点（State、LLM、Signal、Transform、Utility）
- CLI 工具（play、serve、debug、lint、smoke、eval、studio）
- Studio 工作台（Flow / Session / State / Config / Debug / Review / Packages）
- 2 个示例游戏（dnd-adventure、showcase-signal-watch）
...
详细待办见 [docs/internal/todo.md](./docs/internal/todo.md)。
