...
1. **Node（能力层，module名称：node）**：最小执行单元。每个 Node 通过输入/输出 Port 暴露能力，例如读取状态、构建 Prompt、调用 LLM、解析 JSON。
...
Node 是功能实现的最小单位，通过 Port（输入/输出端口）连接形成数据流。
...
Port 是 Node 上的输入/输出端口，用于连接不同的 Node。
...
这意味着像 State 管理类节点的 `key` 参数，虽然大多数情况下会设置默认值，但它本质上是一个输入 Port，也可以通过连线动态传入。
...
**类型检查：**
连接两个 Port 时，引擎会检查类型兼容性。输出端口的类型必须与输入端口的类型匹配。
...
- 子 Flow 的输入/输出端口由子 Flow 顶层的 `inputs`/`outputs` 定义，父 Flow 按普通 Node 的 Port 方式连线
...
- `inputs`：对子 Flow 外部暴露的输入 Port（通常映射到内部 `SignalIn`）
- `outputs`：对子 Flow 外部暴露的输出 Port（可对应多个 `SignalOut`，用于不同出口分支）
- `outputs[].name` 建议与内部 `SignalOut` 的标识一一对应，便于父 Flow 直接按出口名连线（如 `success`、`fallback`、`timeout`）
- 子 Flow 执行完成时，哪个内部 `SignalOut` 被触发，就向父 Flow 的同名输出 Port 发出结果
...
| `object` | ✓ | ✓ | 任意 JSON 对象 |
| `array` | ✓ | ✓ | 任意 JSON 数组 |
| `ChatMessage[]` | ✓ | ✗ | LLM 消息数组（仅 Port） |
| `ImageUrl` | ✓ | ✗ | 图像 URL（仅 Port） |
| 自定义类型 | ✓ | ✗ | 用户通过 TS 定义（仅 Port） |
